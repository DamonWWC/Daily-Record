...
{
    class CaptureImage1Example
    {
        public static async Task RunExample()
        {
            Console.WriteLine("CaptureImage1 Demo (No FFmpeg)");
            Console.WriteLine("=============================");

            try
            {
                // Initialize the CaptureImage1 class
                ...
                if (!string.IsNullOrEmpty(rtspUrl))
                {
                    ...
                    if (captureImage.TestStreamConnection(rtspUrl))
                    {
                        ...
                        string outputPath = Path.Combine("output", "rtsp_frame_opencv.jpg");
                        Console.WriteLine($"Capturing frame to {outputPath}...");

                        bool success = await captureImage.CaptureFrameAsync(rtspUrl, outputPath,10);
                ...
                //        {
                //            string outputFolder = Path.Combine("output", "rtsp_frames_opencv");
                ...
                //string videoPath = Path.Combine("Video", "Õ¾Ì¨·öÌÝ.mp4");
                ...
                //    if (double.TryParse(Console.ReadLine(), out double interval) && interval > 0)
                //    {
                //        string outputFolder = Path.Combine("output", "local_frames_opencv");
