...
namespace Riley.Admin.Tools.Cache
{
    public partial class MemoryCacheTool : ICacheTools
    {
        private static readonly string PatthernRegex = @"\{.*\}";
        ...
        public Task<int> DelAsync(params string[] key)
        {
            foreach (var k in key)
            {
                _memoryCache.Remove(k);
        ...
        public async Task<int> DelByPatternAsync(string pattern)
        {
            if (string.IsNullOrWhiteSpace(pattern))
                return default;
            pattern = Regex.Replace(pattern, PatthernRegex, "(.*)");
        ...
        {
            return GetAllKeys().Where(item => Regex.IsMatch(item, pattern)).ToList();
        }
        ...
        private List<string> GetAllKeys()
        {
            ...
            var coherenState = _memoryCache
                .GetType()
                .GetField("_coherentState", flags)
                .GetValue(_memoryCache);
            var entries = coherenState.GetType().GetField("_entries", flags).GetValue(coherenState);
