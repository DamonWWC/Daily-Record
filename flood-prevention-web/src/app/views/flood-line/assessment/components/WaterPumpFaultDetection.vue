    ...
    <template #content>
      <div class="flex flex-col h-full">
        ...
        <div class="flex flex-row self-center flex-auto">
          <el-button class="px-3 py-1 mx-1" @click="showResult"
            style="background-color: #0a3755; color: #ffffff">查看检测结果</el-button>
          <el-button class="px-3 py-1 mx-1" @click="showWorkOrder"
...
export default {
    ...
    computed: {
        ...
        topic () {
            return JSON.stringify(this.$route.query) === '{}' ? null : `/biz/wia/${this.$route.query.lineId}/${this.$route.query.stationId}/water-pump`;
        }
    },
    methods: {
        onConnected () {
        ...
        unsubscribe (topic) {
            this.businessId = '';
            this.faultDetectionData = [];
            this.$store.dispatch('mqtt/unsubscribe', { topic });
        },
        // getAddress1 (name) {
        //     console.log('routes:', this.$router);
        //     const path = this.$router.options.routes.find(p => p.name === name).path;
        ...
        //     const address = window.location.href.replace(replacestr, path);
        //     console.log('address:', address);
        //     return address;
        // },
        showResult () {
            console.log('route', this.$route);
            const address = getAddress(this.$router.options.routes, 'detectionResult', this.$route.path);
            wpf.emit({
                command: 'showResult',
                param: {
                    type: 'showResult',
        ...
        showWorkOrder () {
            const address = getAddress(this.$router.options.routes, 'createWorkOrder', this.$route.path);
            wpf.emit({
                command: 'showWorkOrder',
                param: {
                    type: 'showWorkOrder',
                    data: {
                        address: address
