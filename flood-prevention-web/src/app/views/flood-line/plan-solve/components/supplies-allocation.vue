    ...
    <div>
        <div class="supplies-allocation w-full h-full flex flex-col justify-start items-start">
            ...
            <div class="content flex-1 w-full flex justify-center items-center">
                ...
                <el-button type="primary" size="mini" :disabled="!isProcessStart"
                    @click="materialApplication">物资调配</el-button>
        ...
        <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" :append-to-body="true" width="72%"
            ...
            <div class="supplies-dialog-body w-full h-full flex flex-col" v-else-if="!isDetail">
                <div class="supplies-dialog-body-head "><span>待我调配</span></div>
        ...
        <supplies-dialog ref="suppliesDialogRef"></supplies-dialog>
    </div>
</template>
...
import SuppliesDialog from '../page-dialog/goods-person-assign-apply-dialog.vue';
export default {
    ...
    computed: {
        dialogTitle() {
            return !this.isView ? '物资申请' : '物资调配记录信息';
        },
        planSolveParam() {
            return this.$store.state.largePassengerFlow.planSolveParam;
        },
        isProcessStart() {
            return this.$store.state.largePassengerFlow.isProcessStart;
        }
    },
    mounted() {
    ...
    methods: {
        ...
        materialApplication() {
            this.$refs.suppliesDialogRef.openDialog();
        },
