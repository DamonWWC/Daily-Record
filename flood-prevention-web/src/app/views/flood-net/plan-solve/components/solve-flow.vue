<template>
    <div class="flex flex-col" style="padding:0 12px">
        ...
        <div class="flex flex-1 content">
            <div class="flex items-center flow-step-begin" v-show="!isStart">
                <el-button class="iconfont icon-bofang" type="primary" size="mini" :disabled="!isReady"
                    @click="handleBeforeProcessStart">启动处置</el-button>
            ...
            <div class="flex flow-step-middle" v-show="isStart">
                <div :class="['flow-step', item.isActive ? 'flow-step--active' : '']" v-for="(item, index) in stepList"
                    ...
                    <div class="flow-step__point">
                        <h3>{{ item.stepName }}</h3>
                        <div
                            v-html="dayjs(item.stepTime).isValid() ? dayjs(item.stepTime).format('YYYY-MM-DD<br>HH:mm:ss') : ''">
                        </div>
                    </div>
        ...
        <el-dialog title="请选择事发位置" :visible.sync="placeDialogVisible" :append-to-body="true">
            <div style="text-align: left; font-size: .875rem;">
                <div>线路名</div><br />
                <el-select v-model="selectedLine" placeholder="请选择线路" style="width:100%">
                    <el-option v-for="item in lineList" :key="item.lineCode" :label="item.lineName"
                        :value="item.lineCode"></el-option>
                </el-select>
            </div>
            <div style="text-align: left; font-size: .875rem;padding-top: 1rem;">
                <div>车站名</div><br />
                <el-select v-model="selectedStation" placeholder="请选择车站" @change="handleStationChange" style="width:100%">
                    <el-option v-for="item in stationList" :key="item.stationCode" :label="item.stationName"
                        :value="item.stationCode"></el-option>
                </el-select>
            </div>
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="handleProcessStartConfirm">确认</el-button>
...
import { BasicDataApi } from '@api/common';
...
export default {
    ...
    data() {
        return {
            ...
            isStart: false,
            isEnd: false,
            placeDialogVisible: false, // 选择事发位置弹窗
    ...
    methods: {
        ...
        async fetchLineList() {
            try {
                const res = await BasicDataApi.fetchLineList();
                this.lineList = res;
            } catch (error) {
                console.log('Error:', error);
            }
        },
        async fetchStationList(lineNo) {
            try {
                const res = await BasicDataApi.fetchStationList({ lineNo });
                this.stationList = res;
                if (this.stationList.length > 0) {
                    this.selectedStation = this.stationList[0].stationCode;
                }
            } catch (error) {
                console.log('Error:', error);
            }
        ...
        handleBeforeProcessStart() {
            if (this.planSolveParam.entrance === 1) {
                this.placeDialogVisible = true;
            } else {
                this.handleProcessStart();
            }
        },
        handleProcessStartConfirm() {
            if (!this.selectedStation) {
                this.$message.error('请选择事发车站');
                return;
            }
            let stationInfo = this.stationList.find(x => x.stationCode === this.selectedStation);
            this.$utils.eventBus.emit('solveFlow/currStation', stationInfo);
        ...
        handleOtherProcess() {
            wpf.emit({
                ...
                param: { data: { planSolveParam: this.planSolveParam } }
            });
        },
        ...
        showDisposalReport() {
            ...
            wpf.emit({
                ...
                param: {
                    ...
                    data: {
                        address: `${address}/${this.planSolveParam.processId}`
                    }
                }
            });
        }
