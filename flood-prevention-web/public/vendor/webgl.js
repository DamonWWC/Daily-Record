...
Texture.prototype.fill = function (width, height, data, pitch) {
    var gl = this.gl;
    gl.bindTexture(gl.TEXTURE_2D, this.texture);
    //requires WebGL 2.0, Chrome 56 or later
    if (gl.UNPACK_ROW_LENGTH) {
        gl.pixelStorei(gl.UNPACK_ROW_LENGTH, pitch);
    } else if (pitch != width) {
        //console.error("gl.UNPACK_ROW_LENGTH requires webgl2 which is not supported");
    }
...
WebGLPlayer.prototype.renderFrame = function (data, pitch, width, height) {
    if (!this.gl) {
    ...
    gl.y.fill(width, height, data[0], pitch[0]);
    gl.u.fill(width >> 1, height >> 1, data[1], pitch[1]);
    gl.v.fill(width >> 1, height >> 1, data[2], pitch[2]);
